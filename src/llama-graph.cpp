...
static bool llm_graph_is_turbo_kv_type(ggml_type type) {
...
static ggml_tensor * llm_graph_prepare_turbo_kv_query(
  ...
  const ggml_tensor  * k,
    ...
    if (!llm_graph_is_turbo_kv_type(k->type)) {
        return q;
    }
...
static ggml_tensor * llm_graph_strip_padded_turbo_v_heads(
  ...
  const ggml_tensor  * v,
              ...
              int64_t  n_head) {
    if (!llm_graph_is_turbo_kv_type(v->type) || v->ne[0] == original_v_head) {
        return cur;
    }
...
ggml_tensor * llm_graph_context::build_attn_mha(
    ...
    if (use_flash_attn) {
        ...
        if (llm_graph_is_turbo_kv_type(v->type)) {
            const bool k_is_turbo = llm_graph_is_turbo_kv_type(k->type);
        ...
        if (llm_graph_is_turbo_kv_type(v_type_kv)) {
            const bool k_is_turbo = llm_graph_is_turbo_kv_type(k->type);
