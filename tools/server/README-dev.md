...
Headers:
- `x-tool-cwd`: optional; sets the working-directory context for the tool, not a containment boundary; this is not part of the tool's params because it is meant to be set by the runtime, not the LLM itself
- `x-tool-runtime`: optional; if present, it must exactly repeat the original `--tools-runtime` value and cannot select or override the destination. If absent, the configured runtime is used. Execution always uses the configured runtime's server-resolved effective target
